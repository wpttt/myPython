...
class Lunar(object):
    #******************************************************************************
    # 下面为阴历计算所需的数�?,为节省存储空�?,所以采用下面比较变态的存储方法.
    #******************************************************************************
    #数组g_lunar_month_day存入阴历1901年到2050年每年中的月天数信息�?
    #阴历每月只能�?29�?30天，一年用12（或13）个二进制位表示，对应位�?1�?30天，否则�?29�?
    g_lunar_month_day = [
    ...
    #数组gLanarMonth存放阴历1901年到2050年闰月的月份，如没有则为0，每字节存两�?
    g_lunar_month = [
    ...
    def sx_year(self): # 返回生肖�?
        ct = self.localtime #取当前时�?
        
        year = self.ln_year() - 3 - 1 # 农历年份�?3 （说明：补减1�?
        year = year % 12 # �?12，得到地支数
    ...
    def gz_day(self): # 返回干支纪日
        ...
        y = ct.year % 100 #取年份后两位（若�?1月�?2月则当前年份减一�?
        y = y - 1 if ct.month == 1 or ct.month == 2 else y
        M = ct.month #取月份（若为1月�?2月则分别�?13�?14来计算）
    ...
    def ln_year(self): # 返回农历�?
        year, _, _ = self.ln_date()
        return year

    def ln_month(self): # 返回农历�?
        _, month, _ = self.ln_date()
        return month

    def ln_day(self): # 返回农历�?
    ...
    def ln_date(self): # 返回农历日期整数元组（年、月、日）（查表法）
        ...
        #阳历1901�?2�?19日为阴历1901年正月初一
        #阳历1901�?1�?1日到2�?19日共�?49�?
        if (delta_days < 49):
    ...
    def ln_date_str(self):# 返回农历日期字符串，形如：农历正月初�?
    ...
    def _julian_day(self):
        ...
        dd = day + 0.5000115740 #本日12:00后才是儒略日的开�?(过一秒钟)*/
...
def run(year, month, day):
    ct = datetime.datetime(year, month, day, 0, 0, 0)
    ln = Lunar(ct)
    #print('公历 {}  北京时间 {}'.format(ln.localtime.date(), ln.localtime.time()))
    #print('{} 【{}�? {}�? {}�? {}�?'.format(ln.ln_date_str(), ln.gz_year(), ln.sx_year(), ln.gz_day(), ln.gz_hour()))
